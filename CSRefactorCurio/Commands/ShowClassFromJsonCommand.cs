...
namespace CSRefactorCurio
{
    ...
    internal sealed class ShowClassFromJsonCommand : BaseCommand<ShowClassFromJsonCommand>
    {
        ...
        protected override async Task ExecuteAsync(OleMenuCmdEventArgs e)
        {
            ...
            if (CSRefactorCurioPackage.Instance == null)
            {
                int t = 0;
                while (CSRefactorCurioPackage.Instance == null && t < 500)
                {
                    await Task.Delay(10);
                    t++;
                }
            ...
            if (CSRefactorCurioPackage.Instance.CurioSolution.Solution == null)
            {
                await CSRefactorCurioPackage.Instance.RefreshSolutionAsync(true);
            }
            ...
            if (proj != null)
                testproj = (CurioProject)CSRefactorCurioPackage.Instance.CurioSolution.Projects.Where((p) => ((CurioProject)p).NativeProject.Equals(proj)).FirstOrDefault();
