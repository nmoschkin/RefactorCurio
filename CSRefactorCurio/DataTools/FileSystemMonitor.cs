...
namespace DataTools.Desktop
{
    ...
    public class FSMonitor : NativeWindow, IDisposable
    {
        ...
        protected SafePtr _Buff = new SafePtr();
        ...
        {
            _hFile = CreateFile(_stor, IO.GENERIC_READ | IO.FILE_READ_ATTRIBUTES | IO.FILE_READ_DATA | IO.FILE_READ_EA | IO.FILE_LIST_DIRECTORY | IO.FILE_TRAVERSE, IO.FILE_SHARE_READ | IO.FILE_SHARE_WRITE | IO.FILE_SHARE_DELETE, IntPtr.Zero, IO.OPEN_EXISTING, IO.FILE_FLAG_BACKUP_SEMANTICS | IO.FILE_FLAG_OVERLAPPED, IntPtr.Zero);
            if (_hFile == (IntPtr)(-1))
            {
                return false;
            }
        ...
        {
            int blen = 0;
            int bufflen = (int)_Buff.Length;
            var tbuff = _Buff.DangerousGetHandle();
                ...
                case FileSystemMonitor.WM_SIGNAL_CLOSE:
                    {
                        ...
                        {
                            MonitorClosed?.Invoke(this, new MonitorClosedEventArgs(MonitorClosedState.ClosedOnError, m.LParam.ToInt32(), null));
                        }
                        else
                        {
